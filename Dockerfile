...
CMD ["sh", "-c", "python3 manage.py collectstatic --noinput --ignore input.css && python3 manage.py migrate && gunicorn config.wsgi:application --bind 0.0.0.0:8000 --workers 3 --timeout 120"]
